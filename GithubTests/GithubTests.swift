...
class NetworkProviderMock: NetworkProviderProtocol {
    ...
    func load<T>(resource: Resource<T>, completion: @escaping (Result<T, NetworkError>) -> Void) {
        ...
        """.data(using: .utf8)!
        if T.self == [User].self {
            let response = try! JSONDecoder().decode(T.self, from: userJson)
...
final class GithubTests: XCTestCase {
    ...
    func testFetchUsersDetails() {
        let expectation = XCTestExpectation(description: "User list request successfully")
        ...
        DispatchQueue.main.async {
            self.wait(for: [expectation], timeout: Timeout.value)
        }
    }
    
    func testFetchRepos() {
        let expectation = XCTestExpectation(description: "User repos request successfully")
    ...
    func testFetchReposFailed() {
        let expectation = self.expectation(description: "List users repos info error tests")
